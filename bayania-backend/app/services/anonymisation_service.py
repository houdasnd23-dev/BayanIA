...
class AnonymisationService:
    ...
    def anonymise(cls, text: str) -> Tuple[str, List[Dict[str, str]]]:
        ...
        # 4. Phone numbers
        phones = list(set(cls.PHONE_REGEX.findall(anonymised_text)))
        phone_idx = 1
        for phone in phones:
            phone_clean = phone.strip()
            # Skip very short matches that are likely false positives (e.g. isolated 2-3 digit numbers)
            digits_only = re.sub(r'\D', '', phone_clean)
            if len(digits_only) < 8:
                continue
            if phone_clean in anonymised_text:
                placeholder = f"[TELEPHONE_{phone_idx}]"
                anonymised_text = anonymised_text.replace(phone_clean, placeholder)
                mappings.append({
        ...
        for name in names:
            ...
            if name_clean in anonymised_text and not any(
                placeholder in name_clean for placeholder in ["[EMAIL", "[CIN", "[TELEPHONE", "[ADRESSE"]
            ):
